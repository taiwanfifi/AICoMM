...
We propose \emph{Scout}, which eliminates KV-cache transmission by exploiting attention alignment within model families: a small edge model's query-to-context attention scores identify the same task-relevant positions as the cloud model (84--92\% overlap across three architectures, $n\!=\!200$, $p > 0.06$), reducing payload from 9.7\,MB to 336~bytes---a 28{,}800$\times$ payload reduction---at the cost of a 57\,ms cloud re-prefill (10$\times$ faster than BF16 transfer at 100\,Mbps).
We design a bandwidth-adaptive protocol with six modes (including a prompt-only baseline) that dynamically selects operating strategy using EWMA-estimated bandwidth on real 5G traces, achieving 100\% deadline compliance via scout fallback with quality within 1.5\% of oracle-optimal mode selection.
Characterization across 9~models from 5~families and 3~tasks (extractive QA, multi-hop reasoning, abstractive summarization) shows INT8 is universally lossless, INT4 fragility is model-specific, and \QtoC selection matches SnapKV while outperforming H2O.
...
\begin{enumerate}
    ...
    \item \textbf{Bandwidth-adaptive 6-mode protocol}: We design a protocol engine that dynamically selects among full-KV, INT8, INT4, mixed-INT4, prompt-only, and scout modes based on real-time channel conditions, with provably optimal single-agent mode selection and bounded quality loss under estimation error (Proposition~1), plus guaranteed multi-agent deadline compliance (Proposition~2). EWMA-based bandwidth estimation is validated across four 5G scenarios: quality remains within 1.5\% of oracle-optimal despite median estimation errors of 11--21\%, and the adaptive policy achieves up to 100\% deadline compliance with graceful quality degradation.
...
Concurrent work C2C~\cite{c2c} enables cross-model KV-cache communication via learned neural fusers, but still transmits multi-megabyte projected representations.
Scout eliminates the need to transmit KV data altogether by transferring only the \emph{positions} where the cloud should attend, achieving 28{,}800$\times$ payload reduction with zero training.
...
\begin{table}[t]
\centering
\caption{Operating modes with quality, bandwidth, and compute characteristics (Qwen-7B, SQuAD~v2, $n = 200$, $\sim$170 avg.\ context tokens). Quality for KV-transfer modes is relative to full BF16 baseline (same model, F1 = 0.608). C-Prefill = cloud re-prefill required.}
\label{tab:operating_points}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Mode} & \textbf{Payload} & \textbf{Quality$^\dagger$} & \textbf{TX@100M} & \textbf{C-Prefill} \\
\midrule
...
\noindent\emph{Notes:}
$^\dagger$\% of same-model full-KV baseline.
$^\ddagger$Prompt-only transmits raw text; the cloud runs full prefill and decode with no eviction, achieving full-KV quality but with no decode-time KV-cache reduction.
$^\S$Scout quality is model-pair dependent.
For 7B$\to$14B at 75\% retention: scout F1 = 0.661, which is 99.4\% of the cloud's own selection (F1 = 0.664) but 90.4\% of the cloud's full-KV baseline (F1 = 0.731).
Scout's advantage over prompt-only is decode-time attention reduction ($O(rn)$ vs.\ $O(n)$ per generated token) and KV-memory savings; this benefit grows with context length and generation length (see Table~\ref{tab:scout_vs_prompt}).
...
Table~\ref{tab:cross_arch_overlap} and Fig.~\ref{fig:cross_arch} extend this analysis across three architectures.
...
\begin{figure}[t]
    ...
    \caption{Cross-architecture position overlap at three retention levels. Same-family pairs (Qwen, Llama, Gemma) consistently achieve 84--92\% overlap at 75\%, while the cross-family pair (Qwen$\to$Mistral) shows lower overlap (73\%), confirming that shared training is the primary driver of attention alignment.}
...
Gemma's scout shows a positive gap ($+0.022$), and Llama's borderline result ($-0.032$, $p = 0.06$) does not reach significance at $\alpha = 0.05$.
We note that Llama's $p$-value is close to the threshold; while the effect is not statistically significant, it may indicate a small quality gap for this pairing that warrants monitoring in deployment.
...
\begin{table}[t]
\centering
\caption{Payload reduction comparison with prior work. Scout substitutes cloud re-computation for KV-cache transfer, unlike data compression methods.}
...
\midrule
CacheGen~\cite{cachegen}  & 3.5$\times$  & KV data  & Data compress. \\
...
Scout (this work)          & \textbf{28,800}$\times$ & Indices & Cloud re-prefill \\
\bottomrule
\end{tabular}
\end{table}
...
We hypothesize that Qwen-7B's lower attention entropy (4.65 nats, Table~\ref{tab:entropy}) produces a more task-relevant position mask than Mistral's own attention, effectively regularizing the context window---an effect analogous to the capacity-matching benefit observed within Qwen (Table~\ref{tab:scout_n200}).
However, Mistral-7B's low baseline F1 (0.258 on SQuAD~v2) limits the practical significance of these results; cross-family transfer under stronger baselines remains an open question.
...
A natural question is why not send the raw prompt ($\sim$4\,KB for 1K tokens in UTF-8) to the cloud, bypassing KV-cache transfer entirely.
We explicitly include this \emph{prompt-only} baseline in Table~\ref{tab:operating_points}: the cloud re-prefills from the original tokens, achieving full-KV quality (100\%) at negligible bandwidth cost.
Scout mode adds one element: the position mask.
This provides three benefits over prompt-only:

\emph{(1)~Decode-time attention and memory reduction.}
Each generated token attends to $rn$ instead of $n$ cached positions, reducing per-step computation by $(1{-}r)$ and decode-time KV memory by the same factor.
Table~\ref{tab:scout_vs_prompt} quantifies this: at 4K context and 256 generated tokens, scout saves 38.8\,ms of decode time (6.7\% reduction) compared to prompt-only, with the savings growing linearly with context length and generation length.
At 32K tokens with 75\% retention, each decode step eliminates 8K key-query dot products per head---a significant per-step saving that compounds over long generations.

\begin{table}[t]
\centering
\caption{Scout vs.\ prompt-only decode cost (Qwen-14B, 75\% retention, 8 KV-heads, 128 head dim). Scout saves $(1{-}r) \times n \times H \times d$ FLOPs per decode step; total saving scales with generation length $G$.}
\label{tab:scout_vs_prompt}
\small
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Context} & \textbf{Gen.} & \textbf{Prompt-only} & \textbf{Scout} & \textbf{Saving} & \textbf{\%} \\
\textbf{$n$} & $G$ & \textbf{decode (ms)} & \textbf{decode (ms)} & \textbf{(ms)} & \\
\midrule
1K    & 64   & 145  & 140  &  4.8  & 3.3\% \\
1K    & 256  & 580  & 561  &  19.4 & 3.3\% \\
4K    & 64   & 189  & 176  &  12.6 & 6.7\% \\
...
\emph{(2)~Enabling the adaptive protocol.}
The edge model's prefill simultaneously produces both the compressed KV-cache (for KV-transfer modes) and the position indices (for scout fallback).
Without the edge model's analysis, the system has only two options: transmit KV data, or send the raw prompt.
The position mask adds scout as an intermediate operating point in the protocol's fallback hierarchy, enabling the bandwidth-quality adaptation that defines our system.

\emph{(3)~Zero-cost eviction guidance.}
When the cloud re-prefills, it generates the full KV-cache in memory.
Scout provides pre-computed eviction guidance: the cloud applies the received mask without running its own selection pass.
This is particularly valuable in multi-turn conversations where the cloud can physically prune unselected entries, reducing KV memory for subsequent turns.
...
\textbf{Proposition 1 (Optimality and robustness of greedy mode selection).}
\emph{Let modes $m_1, \ldots, m_K$ be ordered by decreasing quality with $Q(m_1) \geq \cdots \geq Q(m_K)$ and $S(m_1) \geq \cdots \geq S(m_K)$.}
\emph{(a)~Under known bandwidth $B$, the greedy policy (Eq.~\ref{eq:mode_select}) is optimal.}
\emph{(b)~Under bandwidth estimation error $|\hat{B} - B| \leq \epsilon B$, the quality loss is bounded by one mode step: $Q(\hat{m}^*) \geq Q(m_{j^*+1})$ where $m_{j^*}$ is the oracle-optimal mode, provided $\epsilon < \min_{j} (B_j^{\text{th}} - B_{j+1}^{\text{th}}) / B_j^{\text{th}}$, where $B_j^{\text{th}}$ is the minimum bandwidth for mode $m_j$ to meet the deadline.}

\emph{Proof of (a).}
The greedy policy selects $m^* = m_j$ where $j = \min\{j : T(m_j, B) \leq T_{\max}\}$---the highest-quality feasible mode.
Any alternative feasible mode $m_k$ with $k > j$ satisfies $Q(m_k) \leq Q(m^*)$; any mode with $k < j$ is infeasible.
\emph{Proof of (b).}
If $\hat{B}$ underestimates $B$ by at most $\epsilon$, the estimated bandwidth threshold for mode $m_j$ shifts by at most $\epsilon B_j^{\text{th}}$.
When this shift is smaller than the gap between adjacent thresholds, the estimated policy selects at worst $m_{j^*+1}$---one step below optimal.
Overestimation may select a higher-quality mode whose actual latency exceeds $T_{\max}$; the scout fallback catches this case, bounding quality at $Q(m_K)$.

For our operating points at $T_{\max} = 2$\,s, the minimum threshold gap between adjacent KV-transfer modes is 45\% (INT8 vs.\ Mixed-INT4).
The EWMA estimator with conservative factor 0.8 introduces median error of $-$11\% to $-$21\% (Table~\ref{tab:bw_validation}), well within this margin---confirming that estimation errors cause at most a one-mode quality degradation in practice.
This analysis explains the empirical finding that quality gaps never exceed 1.5\% across all tested scenarios. \hfill$\square$
...
Beyond prefill, scout also reduces per-token decode cost: with $rn$ active positions instead of $n$, each attention step scales as $O(rn)$ rather than $O(n)$.
At 75\% retention with 1024 tokens, this saves $\sim$256 key-query dot products per head per token---a modest per-step gain that compounds over long generations.
...
\begin{figure}[t]
    ...
    \caption{Quality-bandwidth operating points for Qwen-7B ($n = 200$). Each point represents an operating mode. Scout achieves $\sim$90\% of cloud full-KV quality at near-zero bandwidth, with 99.4\% of the cloud's own eviction-based selection quality. The dashed line marks the full-KV baseline.}
...
This approach mirrors adaptive bitrate (ABR) algorithms used in video streaming~\cite{abr}, adapted to the discrete nature of KV-cache operating modes.

\textbf{Robustness to estimation error.}
A key property of the discrete mode set is that the protocol is robust to bandwidth estimation error.
Between adjacent modes (\eg, INT8 at 4.7\,MB and mixed-INT4 at 2.6\,MB), the bandwidth threshold gap is wide: at $T_{\max} = 3$\,s, INT8 requires $\geq$12.5\,Mbps while mixed-INT4 requires $\geq$6.9\,Mbps---a 45\% margin.
The conservative factor ($0.8\times$) provides additional headroom.

We validate this empirically by comparing EWMA-based mode selection against an oracle with perfect bandwidth knowledge across four 5G scenarios (5{,}000 steps each, Table~\ref{tab:bw_validation}).

\begin{table}[t]
\centering
\caption{Bandwidth estimation validation: EWMA ($\alpha\!=\!0.3$, cf$\!=\!0.8$) vs.\ oracle at $T_{\max} = 2$\,s (Qwen-7B, 5{,}000 steps per scenario).}
\label{tab:bw_validation}
\footnotesize
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Scenario} & \textbf{Med.\ Err.} & \textbf{Mode} & \textbf{Quality} & \multicolumn{2}{c}{\textbf{Deadline \%}} \\
                   & \textbf{(\%)} & \textbf{Match} & \textbf{Gap} & \textbf{Est.} & \textbf{Oracle} \\
\midrule
Urban mmWave  & $-$19\%  & 77.9\% & $-$1.5\% & 80.1\% & 100\% \\
Urban sub-6   & $-$21\%  & 66.3\% & $+$0.1\% & 88.9\% & 100\% \\
Suburban mix  & $-$20\%  & 68.1\% & $-$0.6\% & 83.3\% & 100\% \\
...
First, the \emph{quality gap is negligible}: EWMA-based selection achieves within 1.5\% of oracle quality in all scenarios, confirming that the wide inter-mode bandwidth gaps absorb estimation noise.
The negative quality gaps (EWMA slightly \emph{exceeds} oracle in mmWave) occur because the conservative factor occasionally forces scout mode, whose quality occasionally exceeds expectations.
Second, the \emph{deadline gap is the primary cost}: estimation errors reduce deadline compliance by 12--32\,pp relative to the oracle.
This gap is largest in vehicular scenarios (high variability, low autocorrelation) and smallest in sub-6\,GHz (stable bandwidth).
Third, scout fallback bounds the downside: when estimation error causes a deadline miss under KV-transfer modes, the adaptive-with-scout policy recovers 100\% deadline compliance (Table~\ref{tab:protocol_sim}) by falling back to scout mode.

\textbf{EWMA parameter sensitivity.}
Table~\ref{tab:alpha_sensitivity} evaluates $\alpha$ sensitivity in the vehicular scenario (most challenging).

\begin{table}[t]
\centering
\caption{EWMA $\alpha$ sensitivity (vehicular, $T_{\max} = 2$\,s, cf$\!=\!0.8$, 5{,}000 steps). Higher $\alpha$ improves tracking but increases mode switches.}
\label{tab:alpha_sensitivity}
\footnotesize
\begin{tabular}{@{}lcccccc@{}}
\toprule
$\alpha$ & \textbf{Mode} & \textbf{Q Gap} & \textbf{DL} & \textbf{Switches} \\
         & \textbf{Match} & \textbf{(\%)} & \textbf{(\%)} & \textbf{(per 5K)} \\
\midrule
...
Higher $\alpha$ (faster tracking) improves mode match rate from 35\% to 73\% and nearly eliminates quality gap, but at two costs: (1)~more frequent mode switches (579 $\to$ 2{,}602 per 5{,}000 steps), which could cause user-visible quality fluctuations; and (2)~slightly lower deadline compliance because faster tracking reduces conservatism.
The choice of $\alpha = 0.3$ balances these tradeoffs: moderate mode matching with controlled switching frequency.
Importantly, \emph{all} $\alpha$ values achieve quality within 3\% of oracle---the discrete mode structure provides inherent robustness regardless of estimator tuning.

When a catastrophic bandwidth drop occurs (\eg, mmWave blockage to 2\,Mbps), EWMA recovers mode-selection accuracy within 0.4~steps on average (max 2~steps), confirming rapid adaptation.
During blockage, the protocol falls back to scout mode, incurring a quality change from 99.4\% (INT8) to $\sim$90\% (scout) rather than a deadline miss---graceful degradation versus the binary success/failure of static policies.
...
\textbf{Quality-maximizing}: Greedy assignment of bandwidth to the device with the largest marginal quality gain from the next increment.

\textbf{Proposition 2 (Deadline guarantee).}
\emph{For any bandwidth $B_{\text{total}} > 0$ and any number of agents $N$, the Scout protocol guarantees that all $N$ agents meet the deadline $T_{\max}$, provided $T_{\max} > T_{\text{prefill}}^{\text{cloud}} + T_{\text{decode}}$.}

\emph{Proof.}
In the worst case, the protocol assigns each agent to scout mode with payload $S_i \leq 1$\,KB.
Total payload $\sum_i S_i \leq N$\,KB, requiring $T_{\text{TX}} = 8N / B_{\text{total}}$\,seconds.
For practical values ($N \leq 8$, $B_{\text{total}} \geq 1$\,Mbps), $T_{\text{TX}} < 0.1$\,ms.
Each agent's total latency is $T_{\text{prefill}}^{\text{edge}} + T_{\text{TX}} + T_{\text{prefill}}^{\text{cloud}} + T_{\text{decode}} \leq T_{\max}$.
\hfill$\square$

This guarantee holds regardless of bandwidth variability, agent heterogeneity, or mode selection errors---scout mode serves as a universal safety net.
When bandwidth is insufficient for KV-cache transfer within the deadline, devices fall back to scout mode or local inference, ensuring the system never fails silently.

\subsection{Deadline-Aware Fallback Hierarchy}

The complete fallback hierarchy, in decreasing bandwidth order:
\begin{enumerate}
    \item Full BF16 KV-cache transfer (quality: 100\%, 9.7\,MB, no cloud prefill)
    \item INT8 quantized KV-cache (quality: 99.4\%, 4.7\,MB)
    \item Mixed-INT4 quantized KV-cache (quality: 93.6\%, 2.6\,MB)
    \item INT4 quantized KV-cache (quality: 68.7\%, 2.3\,MB)
    \item Prompt-only (quality: 100\%, $\sim$4\,KB, cloud re-prefill, no eviction)
    \item Scout mode with position indices (quality: $\sim$90\%, 336\,B, cloud re-prefill + eviction)
    \item Local edge inference (quality: 60--75\% of cloud, zero bandwidth)
\end{enumerate}
Prompt-only (mode~5) provides full quality but no decode-time KV reduction; scout (mode~6) trades $\sim$10\% quality for $O(rn)$ decode-time attention and KV-memory savings.
Both require cloud re-prefill (57\,ms), making them preferable to KV-transfer under low bandwidth ($<$10\,Mbps) where even INT4 transmission exceeds the deadline.
...
Compared to CacheGen~\cite{cachegen}---the most directly comparable system---Scout differs in three fundamental ways: (1)~Scout transmits position indices (28,800$\times$ payload reduction) rather than compressed KV data (3.5$\times$), at the cost of cloud re-prefill (57\,ms); (2)~Scout adapts across 6~modes in real time (including a prompt-only baseline), whereas CacheGen is static; and (3)~Scout supports cross-model transfer, while CacheGen requires identical sender and receiver models.
...
\textbf{Tasks.}
Primary evaluation uses SQuAD~v2~\cite{squad} (extractive QA, token-F1); secondary evaluation uses HotpotQA~\cite{hotpotqa} (multi-hop reasoning) and XSum~\cite{xsum} (abstractive summarization, ROUGE-1/2/L).
Context lengths range from 100--500 tokens (standard) and up to 4096 tokens (long-context experiments).

\textbf{Sample sizes and statistics.}
All experiments use $n = 200$ samples unless otherwise noted, drawn with fixed random seeds (seed=42 for main experiments, seed=123 for long-context) for reproducibility.
Base models use the prompt format ``Context: [text]$\backslash$nQuestion: [text]$\backslash$nAnswer:'' for all QA tasks.
...
Table~\ref{tab:multitask} extends scout validation to HotpotQA ($n = 100$ per task) and XSum summarization ($n = 200$), both using the 7B$\to$14B pair.

\begin{table}[t]
\centering
\caption{Multitask scout validation (7B$\to$14B; $n = 100$ for QA, $n = 200$ for XSum). F1 for QA, ROUGE-1 for summarization. $\pm$95\% CI.}
\label{tab:multitask}
\small
\begin{tabular}{@{}llcccc@{}}
\toprule
...
\midrule
\multirow{3}{*}{\shortstack[l]{XSum\\(ROUGE-1)}}
...
On XSum summarization ($n = 200$), overlap is even higher than QA (88.3\% at 75\% retention), and scout is statistically indistinguishable from the cloud's own attention at all retention levels ($p > 0.3$).
At 50\% retention, scout marginally \emph{improves} ROUGE-1 by $+0.007$ ($p = 0.049$), suggesting the capacity-matching effect observed in SQuAD generalizes to generation tasks.
This is notable because summarization requires the model to \emph{generate} novel text rather than extract spans, confirming that attention alignment transfers across fundamentally different task types.

\subsection{Long Context Overlap Stability}

Table~\ref{tab:long_ctx} shows position overlap stability as context length increases from 1024 to 4096~tokens ($n = 100$ per configuration, needle-in-haystack SQuAD~v2 with distractor paragraphs).
At 4096~tokens, extracting output attentions from the 14B model would require $\sim$148\,GB (exceeding 80\,GB VRAM), so we measure overlap directly using the 3B$\to$7B pair and validate scout quality for 7B$\to$14B separately.

\begin{table}[t]
\centering
\caption{Long context overlap stability (SQuAD~v2, $n = 100$). $^*$3B$\to$7B pair at 4K (14B output attentions infeasible).}
\label{tab:long_ctx}
\small
\begin{tabular}{@{}llccc@{}}
\toprule
...
Within each pair, overlap at 75\% retention decreases by less than 2~percentage points as context length increases: 83.3\%$\to$82.7\% for 7B$\to$14B (1K$\to$2K), and 81.6\%$\to$80.2\% for 3B$\to$7B (standard$\to$4K, comparing with Table~\ref{tab:scout_n200}).
This confirms that cross-model attention alignment is a structural property rather than a short-context artifact (Fig.~\ref{fig:long_ctx}).

To validate that overlap stability translates to quality, we evaluate 7B$\to$14B scout quality at 4096~tokens (without extracting 14B attentions).
Scout F1 is statistically indistinguishable from full-KV at all retention levels ($p = 0.178$ at 75\%, $p = 0.859$ at 50\%, $p = 0.716$ at 25\%), with scout slightly \emph{exceeding} full-KV at 50\% and 25\% retention---consistent with the capacity-matching effect observed at shorter lengths.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{fig8_long_context_overlap.pdf}
    \caption{Position overlap stability across context lengths. Overlap degrades by ${<}2$\,pp per pair as context doubles, confirming alignment is context-length invariant. Dashed line separates model pairs (7B$\to$14B at 1--2K; 3B$\to$7B at 4K due to VRAM constraints).}
    \label{fig:long_ctx}
\end{figure}
...
\textbf{Model-aware allocation is critical under congestion} (Fig.~\ref{fig:multiagent}).
...
\textbf{Extension to asynchronous arrivals.}
Our synchronized evaluation represents a worst case: all agents compete for bandwidth simultaneously.
Under Poisson arrivals with rate $\lambda$ per agent, the expected number of concurrent requests is $N_{\text{active}} = \lambda \cdot T_{\text{service}}$, where $T_{\text{service}}$ is the average request duration.
At $\lambda = 0.1$\,req/s and $T_{\text{service}} = 2$\,s with $N = 8$ agents, the expected active count is $N_{\text{active}} = 1.6$, substantially reducing contention compared to the synchronized case ($N_{\text{active}} = 8$).
The protocol's mode selection operates per-request: each arriving request observes the current available bandwidth (total minus bandwidth allocated to active requests) and selects accordingly.
Scout fallback guarantees that no arriving request is denied service, regardless of instantaneous contention.
Formal queuing analysis (e.g., M/G/1/K with state-dependent service rates) is an important direction for future work.
...
We note that FlashAttention~\cite{flashattn} and SDPA do not expose per-layer attention weights; practitioners deploying scout must use eager attention during the edge prefill pass.
Since the edge model is small (3--7B) and prefill is a single forward pass, this overhead is acceptable in practice.

\subsubsection{Base Model vs.\ Instruction-Tuned}

The choice between base and instruction-tuned models affects task performance and prompt requirements.
Yi-6B-Chat achieves only F1 = 0.161 on SQuAD~v2 vs.\ Yi-1.5-6B (base) at F1 = 0.659, because chat models generate verbose responses.
To validate that attention alignment extends to production-relevant instruct models, we evaluate Qwen2.5-7B-Instruct $\to$ 14B-Instruct ($n = 200$, ChatML format).
Instruct overlap at 75\% is 85.8\%---comparable to the base model's 83.7\%, confirming alignment survives instruction tuning.
Scout selection is statistically indistinguishable from cloud's own at 75\% ($p = 0.131$) and 50\% ($p = 0.914$), and \emph{significantly improves} at 25\% ($+0.044$ F1, $p < 10^{-4}$, Bonferroni-corrected), reinforcing the attention regularization effect.
Instruct models show larger gaps between position-selected and full-KV inference (0.460 vs.\ 0.772 at 75\%), suggesting broader attention distribution and higher recommended retention ratios for instruct deployments.
...
\textbf{Cloud compute cost.}
Scout trades bandwidth for cloud-side computation: the cloud re-prefills from the original tokens (57\,ms for Qwen-14B at 1K tokens, Table~\ref{tab:scout_latency}).
During this re-prefill, the cloud temporarily generates the full KV-cache in memory.
After applying the position mask, unselected entries can be physically pruned, reducing decode-time KV memory from $n$ to $rn$ positions.
This temporary memory cost is modest (the cloud already has GPU memory to run the 14B model) and is amortized over the multi-token decode phase.

\textbf{Attention regularization effect.}
A recurring finding across our experiments is that scout selection can \emph{improve} cloud quality beyond its own attention-based selection.
This occurs for SQuAD (7B$\to$14B at 50\% and 25\%), XSum (7B$\to$14B at 50\%), cross-family transfer (Qwen$\to$Mistral at 75\%), and instruction-tuned models (7B-Instruct$\to$14B-Instruct at 25\%, $p < 10^{-4}$).
We attribute this to \emph{attention regularization}: the scout's focused attention mask removes noisy positions that the cloud model attends to, effectively acting as a task-relevant filter.
This suggests scout mode offers a dual benefit---bandwidth savings \emph{and} quality improvement---particularly for models with diffuse attention patterns.
...
Second, while we evaluate on extractive QA and abstractive summarization, tasks requiring precise numerical reasoning or multi-step arithmetic may be more sensitive to selection misalignment.
...
Fourth, while we analyze asynchronous arrivals qualitatively (Section~\ref{sec:transport}), a full queuing-theoretic treatment with formal capacity analysis remains future work.
Fifth, we evaluate context lengths up to 4096 tokens; while overlap remains stable at this length, production LLMs routinely handle 8K--128K contexts where attention sparsity patterns may differ.
Sixth, while we validate EWMA bandwidth estimation across four 5G scenarios (Table~\ref{tab:bw_validation}), real-deployment validation with physical hardware and end-to-end packet-level measurement would strengthen the protocol contribution.
Finally, Table~\ref{tab:scout_vs_prompt} shows that scout's decode-time savings over prompt-only transmission are modest at short contexts (3.3\% at 1K tokens) and become significant only at longer contexts (25\% at 32K); at short contexts, the primary value of scout is as a fallback within the adaptive protocol rather than standalone decode efficiency.
...
Expected Attention~\cite{sagekv} estimates future query attention distributions to select positions independent of the current query.
...
More importantly, CacheGen achieves $\sim$3.5$\times$ compression of KV data, while Scout achieves 28{,}800$\times$ \emph{payload reduction} by transmitting position indices instead of KV data---substituting cloud re-computation for bandwidth.
CacheBlend~\cite{cacheblend} fuses precomputed KV segments for RAG via selective recomputation.
Cache-to-Cache (C2C)~\cite{c2c} proposes direct semantic communication between LLMs by projecting a source model's KV-cache into the target model's representation space via learned neural fusers with a gating mechanism that selects beneficial target layers.
C2C tests the same model families (Qwen, Llama, Gemma) and demonstrates 8--10\% accuracy gains over individual models.
Scout differs from C2C in three fundamental ways: (1)~Scout is \emph{training-free}, requiring no learned projections---only position indices---making it immediately deployable; (2)~Scout targets \emph{bandwidth reduction} (28{,}800$\times$ payload reduction) rather than quality improvement; and (3)~Scout transmits 336~bytes of position indices versus C2C's projected KV-cache representations, which remain multi-megabyte.
The two approaches are complementary: C2C improves quality when bandwidth permits KV-cache transfer; Scout eliminates the need for such transfer entirely.
...
By exploiting attention alignment within model families, Scout eliminates KV-cache transmission---reducing payload from megabytes to hundreds of bytes (28{,}800$\times$ payload reduction vs.\ CacheGen's 3.5$\times$ data compression), substituting a 57\,ms cloud re-prefill for multi-second wireless transfer.
...
This overlap remains stable across context lengths (83.3\% at 1K, 82.7\% at 2K, 80.2\% at 4K tokens), across task types (extractive QA, multi-hop reasoning, abstractive summarization), across architectures with different vocabulary sizes, attention head counts, and training corpora, and between base and instruction-tuned model variants (85.8\% for instruct vs.\ 83.7\% for base), confirming it is a structural property of same-family models.

The adaptive protocol engine dynamically selects among six operating modes---from full KV-cache transfer through prompt-only to scout-only operation---based on real-time EWMA bandwidth estimation validated across four 5G scenarios.
Quality loss from estimation error is bounded at 1.5\% (within one mode step of oracle-optimal), and the adaptive policy achieves 98--107\% quality with up to 100\% deadline compliance, while static policies fail under tight deadlines (58\% at 1\,s for INT8).
...
\bibitem{xsum}
S.~Narayan, S.~B.~Cohen, and M.~Lapata, ``Don't Give Me the Details, Just the Summary! Topic-Aware Convolutional Neural Networks for Extreme Summarization,'' in \emph{Proc.\ EMNLP}, 2018.

\bibitem{kvtuner}
X.~Li \etal, ``KVTuner: Sensitivity-Aware Layer-Wise Mixed-Precision KV Cache Quantization for Efficient and Nearly Lossless LLM Inference,'' in \emph{Proc.\ ICML}, 2025.

\bibitem{dynamickv}
X.~Zhou \etal, ``DynamicKV: Task-Aware Adaptive KV Cache Compression for Long Context LLMs,'' in \emph{Proc.\ EMNLP Findings}, 2025.

\bibitem{sagekv}
A.~Devoto \etal, ``Expected Attention: KV Cache Compression by Estimating Attention from Future Queries Distribution,'' \emph{arXiv:2510.00636}, 2025.

\bibitem{quest}
J.~Tang \etal, ``Quest: Query-Aware Sparsity for Efficient Long-Context LLM Inference,'' in \emph{Proc.\ ICML}, 2024.

\bibitem{qhitter}
Z.~Zhang \etal, ``Q-Hitter: A Better Token Oracle for Efficient LLM Inference via Sparse-Quantized KV Cache,'' in \emph{Proc.\ MLSys}, 2024.

\bibitem{pmkvq}
T.~Liu \etal, ``PM-KVQ: Progressive Mixed-Precision KV Cache Quantization for Long-CoT LLMs,'' \emph{arXiv:2505.18610}, 2025.

\bibitem{cacheblend}
J.~Yao \etal, ``CacheBlend: Fast Large Language Model Serving for RAG with Cached Knowledge Fusion,'' in \emph{Proc.\ EuroSys}, 2025.

\bibitem{c2c}
T.~Fu \etal, ``Cache-to-Cache: Direct Semantic Communication Between Large Language Models,'' \emph{arXiv:2510.03215}, 2025.

\bibitem{edgeshard}
M.~Zhang \etal, ``EdgeShard: Efficient LLM Inference via Collaborative Edge Computing,'' \emph{arXiv:2405.14371}, 2024.

\bibitem{sled}
X.~Li \etal, ``SLED: A Speculative LLM Decoding Framework for Efficient Edge Serving,'' \emph{arXiv:2506.09397}, 2025.
...
\bibitem{flashattn}
T.~Dao \etal, ``FlashAttention: Fast and Memory-Efficient Exact Attention with IO-Awareness,'' in \emph{Proc.\ NeurIPS}, 2022.
