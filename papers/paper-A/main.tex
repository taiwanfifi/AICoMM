...
% ============================================================
\begin{abstract}
Efficient key-value (KV) cache management is critical across multiple LLM deployment scenarios: disaggregated serving systems transfer KV-caches between prefill and decode servers~\cite{splitwise,distserve}; long-context inference requires evicting tokens to fit within GPU memory; and edge-cloud collaborative inference transmits KV states across bandwidth-constrained links.
A 14B-parameter model's KV-cache for a 1024-token context exceeds 200\,MB, creating bottlenecks in all three settings.
...
Latency analysis shows that INT8 quantization halves KV-cache transfer time with 3\,ms overhead, yielding a net 44\% latency reduction at 100\,Mbps.
Our findings provide the first comprehensive characterization of KV-cache compressibility across architectures, applicable to disaggregated serving, memory-constrained inference, and edge-cloud systems.
\end{abstract}
...
As large language models (LLMs) scale to billions of parameters, the key-value (KV) cache has emerged as a critical bottleneck across multiple deployment settings.
\emph{Disaggregated serving} systems~\cite{splitwise,distserve,mooncake} separate prefill and decode phases across different GPU pools, transferring KV-caches over datacenter interconnects where even at 100\,Gbps, a 14B model's 200\,MB cache adds measurable latency to time-to-first-token (TTFT).
\emph{Long-context inference} requires fitting KV-caches into limited GPU memory; a 14B model at 4096 tokens consumes over 800\,MB of VRAM for KV storage alone, necessitating token eviction under memory pressure.
\emph{Edge-cloud collaborative inference}~\cite{cachegen} splits computation across devices, requiring KV-cache transmission over bandwidth-constrained links.

In all three settings, KV-cache compression directly improves system performance---reducing transfer latency, fitting longer contexts into memory, or enabling inference over constrained networks.
...
Yet the quality impact of compression varies dramatically across models and methods, and no systematic characterization exists.
...
\begin{enumerate}
    \item \textbf{\QtoC (Query-to-Context) selection}: A task-aware token selection method that uses the query's attention over context positions to identify the most task-relevant tokens. At 25\% retention---the most aggressive operating point for memory-constrained or bandwidth-constrained settings---\QtoC outperforms SnapKV by 29--47\% and H2O by 92--128\% consistently across 4 model families.

    \item \textbf{Diagnostic mixed-precision quantization}: A model-specific recipe that runs per-layer INT4 sensitivity analysis to identify bottleneck layers, then protects them with higher precision. This achieves lossless quality at 3.6$\times$ compression (27.7\% of original size).
